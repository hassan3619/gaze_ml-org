...
if __name__ == '__main__':
    ...
    with tf.Session(config=session_config) as session:
        ...
        current_gaze = np.zeros(2)  # Initialize it as a zero array if gaze isn't calculated
        def _visualize_output():
            global current_gaze
            face_detected = False # Use the global flag
            last_frame_index = 0
            ...
            while True:
                ...
                for j in range(batch_size):
                    ...
                    # Define the maximum number of faces to process
                    max_faces = 1  # Limit to 1 face
                    max_eyes_per_face = 2  # Limit to 2 eyes
                    # Limit the faces to the first `max_faces` detected faces
                    if not face_detected and frame['faces']:  # Check if the first face is detected
                        face_detected = True  # Lock the first face once detected
                        faces_to_process = [frame['faces'][0]]  # Only process the first detected face
                    else:
                        faces_to_process = []  # Do not process any further faces

                    # Iterate through the faces
                for face in faces_to_process:
                    # Process only the faces up to the desired limit
                    face_index = frame['faces'].index(face)  # Get the index of the current face
                    ...
                    eyes_to_process = [eye for eye in frame['eyes'] if eye.get('face_index') == face_index]
                    # Transform predictions
                    for eye in eyes_to_process:
                    # Process only the eyes that are associated with the current face
                        if eye['face_index'] == face_index:
                        # Your existing code to process each eye
                      ...
                      #       thickness=1, lineType=cv.LINE_AA,
                       #  )
                        ...
                        if len(gaze_history) > gaze_history_max_len:
                            ...
                            # Smooth the gaze vector by averaging the history
