...
# at top
# ---- ZMQ JPEG publisher for GUI preview ----
# conda activate gazeml
# cd '/media/hn_97/VOLUME G/GazeML-org/src'
# python elg_run.py

ELG_PUB_ENDPOINT = os.environ.get("ELG_PUB", "tcp://127.0.0.1:5556")
_elp_ctx = zmq.Context.instance()
_elp_pub = _elp_ctx.socket(zmq.PUB)
_elp_pub.bind(ELG_PUB_ENDPOINT)

def publish_elg_frame(bgr, jpg_quality=65, max_w=320):
    """Downscale & JPEG-encode then publish one frame to the GUI."""
    try:
        h, w = bgr.shape[:2]
        if w > max_w:
            scale = max_w / float(w)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--camera_id', type=int, default=3, help='ID of webcam to use')
