...
import socket, json
import argparse, cv2
from face_lock import FaceLockFilter

import coloredlogs
import cv2 as cv
import numpy as np
import tensorflow as tf
from user_interface import select_user, load_db
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target", type=str, default=None, help="Operator name to lock on")
    ...
    parser.add_argument("--target", type=str, default=None, help="Operator name to lock on")
    parser.add_argument("--match-thresh", type=float, default=0.50, help="Cosine threshold for recognition")
    parser.add_argument("--det-interval", type=int, default=6, help="Frames between full detect+embed")
    ...
    def _load_user_calib(user: str):
        path = os.path.join("profiles", f"{user}_calib.json")
    ...
    with tf.Session(config=session_config) as session:
        ...
        selected_operator = select_user()
        db = load_db()
        operator_embedding = db.get(selected_operator, None)

        def filter_operator_faces(frame):
            rgb = cv.cvtColor(frame, cv.COLOR_BGR2RGB)
            encs = face_recognition.face_encodings(rgb)
            locs = face_recognition.face_locations(rgb)
            for enc, loc in zip(encs, locs):
                matches = face_recognition.compare_faces([operator_embedding], enc)
                if matches[0]:
        ...
        def _visualize_output():
            ...
            while True:
                ...
                for j in range(batch_size):
                    ...
                    # after you compute/average current_gaze (theta, phi)
                    theta, phi = float(current_gaze[0]), float(current_gaze[1])
                    # map to roughly [-1, 1] (tune scales from quick calibration)
                    gx = np.clip(phi / 0.35, -1.0, 1.0)     # horizontal
                    gy = np.clip(theta / 0.35, -1.0, 1.0)   # vertical
