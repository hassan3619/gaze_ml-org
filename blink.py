...
def blink_events(camera_index: int = 0, visualize: bool = False, yield_double: bool = False,
    ...
    Extras added:
      • Prints "consecutive" if two DOUBLE blinks occur within 1 ms.
    ...
    try:
        while True:
            ...
            if L_ear_raw is not None and R_ear_raw is not None:
                ev = det.update(L_ear_raw, R_ear_raw, now_ms=time.time() * 1000.0)
                ts = time.time()  # seconds (float)
                ...
                if ev["double_blink"]:
                    ...
                    # ----- consecutive LEFT within 1 second -----
                    ...
                    # ----- consecutive RIGHT within 1 second -----
